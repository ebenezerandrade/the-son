...
\section{Aplicação do catálogo}


Para a aplicação do catálogo foi realizado a aplicação de cinco personas com intuito de simular cinco ambientes isolados de desenvolvimento de software utilizando como base de sua arquitetura o padrão arquitetural MVC.

\subsection{Persona 1}
\label{subsec:persona1}

Está subseção detalha o processo de aplicação do catálogo de segurança para a primeira persona 
...
Portanto a gem selecionada para ser aplicada ao projeto por Heleno é a devise, pois além da posição do ranking é a gem mais recomendada pela comunidade Rails.As vantagens da utilização do devise é que (i) é uma solução baseada no padrão arquitetural MVC, (ii) permite a criação e a utilização de vários perfis de usuários conectados ao mesmo tempo, (iii) basea-se no conceito de modularidade, utilizando somente o que é necessário. Essa \textit{gem} possui 10 módulos: 
...
Com base nas descrições dos módulos e a capacidade deles em satisfazer a utilização da \textit{gem} devise, a gem e seus módulos foram então modelados como operacionalizações.
...
As operacionalizações filhas de "utilizar gem devise" e que atendem a demanda da \textit{Persona} 1 são:

\begin{itemize}
	\item habilitar Database Authenticable: É um dos requisitos mínimos para que ocorra a persistência da senha no banco de dados.
	\item habilitar Recoverable: Capacidade de redefinir a senha do usuário.
	\item habilitar Registrable: É um dos requisitos mínimos e que permite a inscrição de usuários. 
	\item habilitar Trackable: monitorar a quantidade de entradas do usuário. 
	\item habilitar Validatable: Validar conta via email do usuário. 
\end{itemize}

portanto ao habilitar os módulos eles são satisfeitos e podemos verificar o nível em que segurança pode ser satisfeito com a utilização da gem devise. A implementação dos padrões de projeto strategy, observer e composite já estão satisfeitos pois a aplicação é feita em MVC e a gem que está sendo utilizada também percorre todas as camadas e é voltada para o padrão arquitetural MVC. 

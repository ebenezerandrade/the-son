...
A notação no SIG para a \textbf{meta flexível} é um símbolo similar ao de uma nuvem, conforme apresentado pela figura \ref{fig01}. Esse simbolo, possui pequenas variações na forma, que representam uma \textbf{operacionalização} (nuvem em negrito), e também uma \textbf{reivindicação} (nuvem tracejada).
...
As relações que as metas flexíveis possuem uma com as outras é estabelecida através de \textit{links} de interdependências, sendo esses \textit{links} que realizam o registro da decomposição das metas flexíveis em metas flexíveis mais específicas (filhos). De certa forma, a decomposição em outras metas, contribui para o nível de satisfação da meta flexível mais genérica (pai). Os tipos de contribuições são apresentados na tabela \ref{tiposDeContribuições} adaptado de  \cite{chung2012non}.
...
Os \textit{links} entre as metas flexíveis, representam a contribuição que uma meta flexível tem com uma outra meta flexível, logo, a figura \ref{catalogoDeAvaliação}, apresenta a forma como as \textit{labels} são propagadas, de acordo com seu tipo de relacionamento. Para facilitar o entendimento, utilizou-se a decomposição de 'integridade' em 'precisão' e 'completude', através dos \textit{links} AND(E) e OR(OU). Portanto, pode-se verificar que: 
...
\begin{figure}[h!]
	...
	\caption{Propagação das \textit{labels} para os diferentes tipos de contribuição. Fonte \cite{chung2012non} adaptado.}
...
\begin{table}[h!]
	...
	\begin{tabular}{|c|c|c|}
		...
		\begin{tabular}[c]{@{}c@{}}Da lama à \\ estrutura\end{tabular} & \begin{tabular}[c]{@{}c@{}}Da suporte ao desenvolvedor em evitar um \\ "mar" de componentes ou objetos. Os padrões\\ dessa categoria suportam uma decomposição\\  controlada de uma tarega geral em\\  subtarefas cooperantes.\end{tabular} & \begin{tabular}[c]{@{}c@{}}Padrão Layers\\ \\ \\ Padrão \\ Pipe-and-Filter\\ \\ \\ Padrão Blackboard\end{tabular} \\ \hline
...
Devido ao foco ser a construção de um catálogo de segurança para o padrão arquitetural MVC, a subseção \ref{subsec:mvc}, detalha este padrão arquitetural. 


\subsection{MVC - Model-View-Controller}
\label{subsec:mvc}

O MVC divide a aplicação em três componentes: (i) a \textit{Model}, onde contém os dados e as funcionalidades, entendida como a camada de manipulação dos dados , (ii) a \textit{View}, componente responsável por apresentar as informações para o usuário, (iii) a \textit{Controller}, é responsável por receber e controlar as requisições de entrada do usuário, realizando o controle de qual \textit{model} usar e qual \textit{view} será mostrada ao usuário \cite{buschmann1996system}.  
...
\begin{table}[h!]
	...
	\begin{tabular}{|l|l|}
		...
		\begin{tabular}[c]{@{}l@{}}- Fornece o núcleo funcional da aplicação\\ - Registra visualizações e controladores dependentes\\ - Notifica componentes dependentes sobre mudanças de dados\end{tabular} & \begin{tabular}[c]{@{}l@{}}\textit{View}\\ \textit{Controller}\end{tabular} \\ \hline
	\end{tabular}
\end{table}
...
\begin{table}[h!]
	...
	\begin{tabular}{|l|l|}
		\hline
		\textbf{Responsabilidades} & \textbf{Colaboradores} \\ \hline
		\begin{tabular}[c]{@{}l@{}}- Cria e inicializa o controlador associado\\ - Exibe informações ao usuário\\ - Implementa o procedimento de atualização\\ - Recupera dados do modelo\end{tabular} & \begin{tabular}[c]{@{}l@{}}\textit{Model}\\ \textit{Controller}\end{tabular} \\ \hline
	\end{tabular}
\end{table}

\subsubsection{Controller}

A \textit{Controller} trata as entradas do usuário como eventos, as controladoras desses eventos recebem um pedido da \textit{View}, onde um procedimento específico na \textit{Controller} trata esse pedido e realiza uma chamada de um procedimento na \textit{model}. A \textit{Controller} basicamente traduz os pedidos de eventos para a model e a cada pedido existe uma \textit{view} associada. 
...
\begin{table}[h!]
	...
	\begin{tabular}{|l|l|}
		\hline
		\textbf{Responsabilidades}                    & \textbf{Colaboradores}                                                        \\ \hline
		\begin{tabular}[c]{@{}l@{}}- Aceita entrada do usuário como eventos\\ - Traduz os eventos para a \textit{Model} ou exibir os pedidos para a \textit{View}\\ - Se necessário, implementa prodecimentos de atualização\end{tabular} & \begin{tabular}[c]{@{}l@{}}\textit{Model}\\ \textit{View}\end{tabular} \\ \hline
	\end{tabular}
\end{table}

...
Neste capítulo, é apresentado as primeiras aplicações do catálogo, para isso então foi utilizado o conceito de personas (XXcolocar linkXX) buscando demonstrar os cenários em que o catálogo é aplicável. Portanto, realizou-se a simulação de cinco cenários para validar a aplicação do catálogo.

%Para tornar mais compreensível a apresentação dos resultados, dividiu-se o capitulo em partes representando cada persona, onde é detalhado pela descrição da persona, identificação de uma possível solução, para trazer o leitor o entendimento do contexto de aplicação do catálogo e por fim a aplicação do catálogo e a tomada de decisões.  
